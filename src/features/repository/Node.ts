...
class MegapolosNode extends BaseRepository<NodeTable> {
    ...
    for (let i in containers) {
      ...
      for (let j in volumes) {
        ...
        containerResult.volumes.push({
          source: (await volume.volume.getData()).outer_path,
          target: volume.containerVolume.inner_path,
        });
  ...
  async getUsedPorts() {
    return (
      await new Container(this.ctx).getByFields({ node_id: this.id })
    ).map((app) => app.outer_port);
  }
